...
struct TastingDetailView: View {
    ...
    init(
        ...
        _model = State(initialValue: TastingDetailModel(tastingId: tastingId, seed: seed))
    }
    ...
    private func prefetchDetailImages(_ tasting: TastingDetail) {
        var urls: [URL] = []
        if let s = tasting.userAvatarUrl, let u = URL(string: s) {
            urls.append(u)
        }
        if let s = tasting.bottleImageUrl, let u = URL(string: s) {
            urls.append(u)
        }
        if let s = tasting.imageUrl, let u = URL(string: s) {
            urls.append(u)
        }
