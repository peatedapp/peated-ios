            ...
            case let .json(payload):
                return payload.results.map { item in
                    ...
                    return Bottle(
                        ...
                        totalRatings: Int(b.totalTastings),
...
public extension CollectionRepository {
    func addBottleToFavorites(bottleId: String, user: String = "me") async throws {
    ...
    func removeBottleFromFavorites(bottleId: String, user: String = "me") async throws {
        guard let collectionId = try await getFavoritesCollectionId(user: user) else {
            throw APIError.requestFailed("Favorites collection not found")
        }
        let client = await client
