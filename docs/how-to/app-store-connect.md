...
- Guideline 1.2 needs report and block controls for user-generated content. Report lives in the `OverflowMenu` of tastings, member reviews, comments, and profiles as a `ReportMenuItem`, which opens the one report sheet mounted at the app root and calls `POST /reports`; Block and Unblock live on profiles and in Settings under Blocked Members. The server does not hide a blocked member's content, so `BlockList` filters feeds, search, and comments on the device.
