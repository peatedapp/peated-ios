...
public struct CacheConditionalsMiddleware: ClientMiddleware {
  ...
  public func intercept(
    ...
    if let key {
      var etag: String? = nil
      var lastModString: String? = nil
      if let name = HTTPField.Name("ETag") { etag = resp.headerFields[name] }
      if let name = HTTPField.Name("Last-Modified") { lastModString = resp.headerFields[name] }
      var meta = await NormalizedStore.shared.metadata(for: key) ?? CacheMetadata()
      if let etag { meta.etag = etag }
      if let lastModString { meta.lastModified = lastModString.toDateRFC1123() }
...
// RFC1123 helpers
extension Date {
  fileprivate var rfc1123String: String {
...
extension String {
  fileprivate func toDateRFC1123() -> Date? {
    let fmt = DateFormatter(); fmt.locale = .init(identifier: "en_US_POSIX"); fmt.timeZone = .init(secondsFromGMT: 0)
