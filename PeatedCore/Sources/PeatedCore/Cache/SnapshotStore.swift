...
public enum SnapshotStore {
    private static func userSnapshotKey(_ id: String) -> CacheKey {
        CacheKey("userSnapshot:\(id)")
    }

    private static func userRecentKey(_ id: String) -> CacheKey {
        CacheKey("userRecent:\(id)")
    }

    private static func bottleSnapshotKey(_ id: String) -> CacheKey {
        CacheKey("bottleSnapshot:\(id)")
    }

    private static func entitySnapshotKey(_ id: String) -> CacheKey {
        CacheKey("entitySnapshot:\(id)")
    }
    ...
    public static func appendUserRecent(userId: String, tastingIds: [String], cap: Int = 20) async {
        ...
        let current: [String] = await NormalizedStore.shared.get(key, as: [String].self)?.0 ?? []
...
public extension SnapshotStore {
    static func merge(_ seed: UserProfileSnapshot?, _ cached: UserProfileSnapshot?) -> UserProfileSnapshot? {
        ...
        if let v = c?.pictureUrl {
            out.pictureUrl = v
        }
        if let v = c?.tastingsCount {
            out.tastingsCount = v
        }
        if let v = c?.bottlesCount {
            out.bottlesCount = v
        }
        if let v = c?.collectedCount {
            out.collectedCount = v
        }
        if let v = c?.contributionsCount {
            out.contributionsCount = v
        }
        if let v = c?.friendStatus {
            out.friendStatus = v
        }
        return out
    }

    static func merge(_ seed: BottleSnapshot?, _ cached: BottleSnapshot?) -> BottleSnapshot? {
        guard seed != nil || cached != nil else { return nil }
    ...
    static func merge(_ seed: EntitySnapshot?, _ cached: EntitySnapshot?) -> EntitySnapshot? {
        guard seed != nil || cached != nil else { return nil }
