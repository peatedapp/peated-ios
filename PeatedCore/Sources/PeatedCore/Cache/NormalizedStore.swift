...
public actor NormalizedStore {
  ...
  public func remove(_ key: CacheKey) { values.removeValue(forKey: key); metas.removeValue(forKey: key); scheduleSave(); metrics.removes += 1 }
  public func clear() { values.removeAll(); metas.removeAll(); scheduleSave(); metrics.clears += 1 }
  ...
  public func pruneExpired(now: Date = Date()) {
    let keysToRemove = metas.compactMap { (key, meta) -> CacheKey? in
      if let exp = meta.expiry, exp <= now { return key }
      return nil
    }
    for k in keysToRemove { values.removeValue(forKey: k); metas.removeValue(forKey: k) }
    if !keysToRemove.isEmpty { scheduleSave(); metrics.prunedExpired += keysToRemove.count }
  }
  ...
  public func pruneByPrefix(prefix: String, maxEntries: Int) {
    ...
    for k in toRemove { values.removeValue(forKey: k); metas.removeValue(forKey: k) }
    if !toRemove.isEmpty { scheduleSave(); metrics.prunedPrefix += toRemove.count }
  }
  ...
  public func pruneTotal(maxEntries: Int) {
    ...
    for k in toRemove { values.removeValue(forKey: k); metas.removeValue(forKey: k) }
    if !toRemove.isEmpty { scheduleSave(); metrics.prunedTotal += toRemove.count }
  }

  /// Snapshot current metrics (consider exporting to Sentry later)
  public func metricsSnapshot() -> Metrics { metrics }
